		...
		<div class="container">
			...
			<h2>발주 관리</h2>
				<div id="buy" class="tab-pane fade active show in">
					<div class="col-md-15">
					    <div class="form-area">
						    <form id="searchFrm" action="${pageContext.request.contextPath }/ERP/searchRequest.do">  
								<table id="purchaseLog-table" class="table">
									<thead>
											<tr>
												...
												<th>
												  	<button type="submit" class="btn btn-default" id="searchRequest">검색</button>
												</th>
												<th>
											...
											<c:if test="${ not empty list }">
												<c:forEach items="${ list }" var="request">
													<tr>
														...
														<td>
															<c:if test="${ request.confirm eq 0}">대기</c:if>
															<c:if test="${ request.confirm eq 1}">완료</c:if>
															<c:if test="${ request.confirm eq -1}">취소</c:if>
														</td>
													</tr>
												</c:forEach>
											</c:if>
